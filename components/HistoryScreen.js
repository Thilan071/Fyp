...
const HistoryScreen = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        // Make sure to include imageUrls in the pushed object
        const data = doc.data();
        historyData.push({
          ...data,
          imageUrls: data.imageUrls || [] // Ensure there's a fallback if imageUrls is not present
        });
      });
  ...
  {history.map((item, index) => (
    <View key={index}>
      ...
      {item.imageUrls.map((url, urlIndex) => (
        <Image key={urlIndex} style={styles.image} source={{ uri: url }} />
      ))}
    </View>
  ))}
</View>
